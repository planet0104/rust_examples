use neat::utils::{ random_int, random_float, random_usize};
use neat::genes::{ Genome,NeuronGene,LinkGene,Innovation };
...
impl GA {
                ...
                while num_to_spawn>0 {
                    ...
                    if !chosen_bestyet{
                        ...
                        if baby.num_neurons() < params::MAX_PERMITTED_NEURONS {
                            baby.add_neuron(params::CHANCE_ADD_NODE,
                                        &mut self.innovation, params::NUM_TRYS_TO_FIND_LOOPED_LINK);
                        }
                        //加入链接的几率
                        baby.add_link(params::CHANCE_ADD_LINK,
                                    params::CHANCE_ADD_RECURRENT_LINK,
                                    &mut self.innovation,
                                    params::NUM_TRYS_TO_FIND_LOOPED_LINK,
                                    params::NUM_ADD_LINK_ATTEMPTS);
                        //对权重实行突变
                        baby.mutate_weights(params::MUTATION_RATE,
                                        params::PROBABILITY_WEIGHT_REPLACED,
                                        params::MAX_WEIGHT_PERTURBATION);
                        //对激励响应实行突变
                        baby.mutate_activation_response(params::ACTIVATION_MUTATION_RATE,
    ...
    pub fn crossover(&mut self, mum:&Genome, dad:&Genome) ->Genome{
        ...
        //最后创建基因
        let baby_genome = Genome::from(self.next_genome_id, baby_neurons, baby_genes, mum.num_inputs(), mum.num_outputs());
        self.next_genome_id += 1;
    ...
    fn speciate_and_calculate_spawn_levels(&mut self){
        ...
        for gen in 0..self.genomes.len(){
            ...
            for spc in 0..self.species.len(){
                let campatibility = self.genomes[gen].get_compatibility_score(self.species[spc].leader());
                //如果这个体类似于这个物种添加到物种
            ...
            if spc_selected != -1{
                ...
                self.species.push(Species::new(&self.genomes[gen], gen, self.next_species_id));
    ...
    pub fn render_species_info(&self, surface: ui::Surface, mut left:i32, top:i32, right:i32, bottom:i32){
        ...
        for spc in 0..self.species.len(){
            //选择画刷
            let color = ui::rgb(num_colours*spc as i32, 255, 255-num_colours*spc as i32);
            if spc == self.species.len()-1{
                ui::rectangle(surface, left, top, right, bottom, color);
            }else{
                ui::rectangle(surface, left, top, (left as f64+slice_per_sprite*self.species[spc].num_members() as f64) as i32, bottom, color);
            }
            ...
            if self.species[spc].best_fitness() == self.best_ever_fitness{
                ...
                s = format!("无改善基因数: {}", self.species[spc].gens_no_improvement());
