...
use neat::utils::{ sqrt_usize, random_float, random_int, random_usize, random_clamped, random_clamped_f64 };
use neat::phenotype::{Neuron, Link, NeuralNet};
use std::rc::Rc;
...
impl Ord for LinkGene {
    fn cmp(&self, other: &LinkGene) -> Ordering {
        self.innovation_id.partial_cmp(&other.innovation_id).unwrap()
    }
}
...
impl SInnovation{
    ...
    pub fn with_split(_in:i32, out:i32, t:InnovationType, inov_id:i32, neuron_type:NeuronType, x:f32,y:f32) -> SInnovation{
        SInnovation{
            neuron_in: _in,
            neuron_out: out,
            innovation_type: t,
...
impl Innovation{
    ...
    fn new_ctor(&mut self, start_genes: &Vec<LinkGene>, start_neurons:&Vec<NeuronGene>){
        ...
        for nd in start_neurons {
            self.innovs.push(SInnovation::from(nd, self.next_innovation_num, self.next_neuron_id));
            self.next_innovation_num+=1;
            self.next_neuron_id+=1;
        }
        //添加连接
        for gen in start_genes {
            let new_innov = SInnovation::new(gen.from_neuron,
                                gen.to_neuron,
    ...
    pub fn check_innovation(&self, _in:i32, out:i32, iv_type:InnovationType) -> i32{
        ...
        for inv in &self.innovs {
            if inv.neuron_in==_in &&
                inv.neuron_out==out &&
                inv.innovation_type == iv_type {
    ...
    pub fn create_new_innovation_with_pos(&mut self, from:i32, to:i32, iv_type:InnovationType, neuron_type:NeuronType, x:f32, y:f32) ->i32 {
        let new_neuron = iv_type == InnovationType::NewNeuron;
        let mut new_innov = SInnovation::with_split(from, to, iv_type, self.next_innovation_num, neuron_type, x, y);
        if new_neuron {
            new_innov.neuron_id = self.next_neuron_id;
            self.next_neuron_id+=1;
        }
...
impl Genome {
    ...
    pub fn minimal(id:i32, inputs:usize, outputs:usize) ->Genome{
        ...
        for i in 0..inputs{
            gen.neurons.push(NeuronGene::new(NeuronType::Input, i as i32, 0.0, (i as f32+2.0)*input_row_slice, false));
        }
        //创建偏移
        gen.neurons.push(NeuronGene::new(NeuronType::Bias, inputs as i32, 0.0, input_row_slice, false));

        //创建输出神经元
        let output_row_slice = 1.0/(outputs as f32+1.0);
        for i in 0..outputs{
            gen.neurons.push(NeuronGene::new(NeuronType::Output, (i+inputs+1) as i32, 1.0, (i as f32+1.0)*output_row_slice, false));
        }
        ...
        for i in 0..inputs+1{
            for j in 0..outputs{
                gen.links.push(LinkGene::new(gen.neurons[i].id,
    ...
    pub fn from(id:i32, neurons:Vec<NeuronGene>, genes:Vec<LinkGene>, inputs:usize, outputs:usize) -> Genome{
        Genome{
            genome_id: id,
            neurons: neurons,
            links: genes,
    ...
    pub fn create_phenotype(&mut self, depth:i32) {
        ...
        if self.neurons[0].neuron_type == NeuronType::Input {
            for n in &self.neurons{
                neurons.push(Neuron::new(n.neuron_type.clone(),
                            n.id,
                            n.split_y as f64,
                            n.split_x as f64,
            ...
            for n in 0..self.neurons.len(){
                neurons.push(Neuron::new(self.neurons[self.neurons.len()-1-n].neuron_type.clone(),
    ...
    pub fn add_link(&mut self, mutation_rate:f32, chance_of_lopped: f32, innovation: &mut Innovation, mut num_trys_to_find_loop:i32, mut num_trys_to_add_link:i32) {
        //根据突变率来确定是否立即返回
        if random_float() > mutation_rate{ return; }
        ...
        if random_float() < chance_of_lopped {
            ...
            while num_trys_to_add_link>0 {
                ...
                id_neuron2  = self.neurons[random_int(self.num_inputs as i32+1, self.neurons.len() as i32-1) as usize].id;
                if id_neuron2 == 2 { //????
                    continue;
                }
        ...
        if id<0 {
            ...
            let new_gene = LinkGene::create(id_neuron1, id_neuron1, true, id, random_clamped_f64(), recurrent);
            self.links.push(new_gene);
        }else{
            //次创新已存在，下面要做的就是使用已存在的创新标识来创建新基因
            let new_gene = LinkGene::create(id_neuron1, id_neuron2, true, id, random_clamped_f64(), recurrent);
            self.links.push(new_gene);
        }
    }

    //增加一个神经细胞
    pub fn add_neuron(&mut self, mutation_rate: f32, innovations: &mut Innovation, mut num_trys_to_find_old_link:i32) {
        //根据突变率来确定是否返回
        ...
        if self.links.len() < size_threshold {
            while num_trys_to_find_old_link>0{
                ...
                chosen_link = random_int(0, self.num_genes() as i32-1-sqrt_usize(&self.num_genes())as i32) as usize;
        ...
        let new_depth = (self.neurons[self.get_element_pos(from)].split_y +
                            self.neurons[self.get_element_pos(to)].split_y)/2.0;
        let new_width = (self.neurons[self.get_element_pos(from)].split_x +
                            self.neurons[self.get_element_pos(to)].split_x)/2.0;
        ...
        */
        ...
        if id<0 { //这是一个新的创新
            //为新的神经细胞加入创新
            let new_neuron_id = innovations.create_new_innovation_with_pos(from, to, InnovationType::NewNeuron,
                                                                    NeuronType::Hidden,
                                                                    new_width,
                                                                    new_depth);
            //创建新的神经细胞基因并将它加入基因组
            self.neurons.push(NeuronGene::new(NeuronType::Hidden, new_neuron_id, new_depth, new_width, false));
            ...
            //为两个新连接基因生成创新标识号
            let id_link1 = innovations.check_innovation(from, new_neuron_id, InnovationType::NewLink);
            ...
            self.links.push(link2);
            //创建新的神经细胞
            let new_neuron = NeuronGene::new(NeuronType::Hidden, new_neuron_id, new_depth, new_width, false);
            //并将它加入基因组
    ...
    pub fn get_compatibility_score(&self, genome:&Genome)->f64{
        ...
        while (g1<link_len-1) || (g2<link_len-1) {
            ...
            if id1 == id2{
                ...
                num_matched+=1.0;
                //得到这两个基因之间的权重差
                weight_difference += (self.links[g1 as usize].weight - genome.links[g2 as usize].weight).abs();
            }
    ...
    fn duplicate_link(&self, neuron_in:i32, neuron_out:i32)->bool{
        for gene in &self.links{
            if gene.from_neuron == neuron_in && 
                gene.to_neuron == neuron_out{
                    //已经有了此连接
                    return true;
                }
        }
