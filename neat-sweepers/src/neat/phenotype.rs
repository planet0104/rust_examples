// Desc：创建神经网络所需的定义。
use neat::genes::{NeuronType};
use neat::utils::{ clamp};
use win::ui;
...
impl NeuralNet{
    ...
    //更新此时钟周期的网络
    pub fn update(&mut self, inputs:&Vec<f64>, run_type:RunType) ->Vec<f64>{
        //创建一个用来存放outputs的Vec
    ...
    pub fn draw_net(&mut self, surface: ui::Surface, left:i32, top:i32, right:i32, bottom:i32){
            ...
            for lnk in &neuron.links_out{
                ...
                if !lnk.recurrent && !bias{
                    ...
                    if start_x==end_x && start_y == end_y{
                        ...
                        ui::ellipse(surface, (x-rad_link)as i32, (y-rad_link) as i32, (x+rad_link) as i32, (y+rad_link)as i32);
                        ui::select_pen(surface, temp_pen);
                        ui::delete_pen(pen);
                    }else{
                        let mut thickness = lnk.weight.abs() as f32;
        ...
        for neuron in &self.neurons{
            ...
            ui::ellipse(surface, (x-rad_neuron)as i32, (y-rad_neuron) as i32, (x+rad_neuron) as i32, (y+rad_neuron)as i32);
        }
